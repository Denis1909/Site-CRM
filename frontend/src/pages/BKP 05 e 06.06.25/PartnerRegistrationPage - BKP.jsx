...
function PartnerRegistrationPage() {
  ...
  // NÃO MEXER NESSES, SERÃO USADOS EM PF
  ...
  // Contato Principal (agora dentro de "Dados da Pessoa" para PF)
  const [contactName, setContactName] = useState('');
  // O contactPhone e contactEmail são os mesmos que 'phone' e 'email' acima?
  // Vou assumir que 'phone' e 'email' são os contatos principais, e 'contactName' é o nome do contato.
  // Se forem campos separados, avise-me! Por enquanto, estou mantendo 'phone' e 'email' como os principais do PF.
  // const [contactPhone, setContactPhone] = useState(''); // Este é redundante se usar 'phone'
  // const [contactEmail, setContactEmail] = useState(''); // Este é redundante se usar 'email'
  ...
  const handleRegisterPartner = (event) => {
    ...
    console.log('Dados do Parceiro (Lead):', {
      partnerCategory,
      partnerName: partnerCategory === 'PJ' ? partnerName : partnerName, // Razão Social ou Nome do Contato
      fantasyName: partnerCategory === 'PJ' ? fantasyName : undefined, // Apenas para PJ
      legalResponsible: partnerCategory === 'PJ' ? legalResponsible : undefined, // Apenas para PJ
      cnpjCpf: partnerCategory === 'PJ' ? cnpjCpf : cnpjCpf, // CNPJ ou CPF
      ieRg: partnerCategory === 'PJ' ? ieRg : ieRg, // IE ou RG
      phone,
      email,
      status,
      street,
      ...
      state,
      zipCode,
      contactName: partnerCategory === 'PF' ? contactName : undefined, // Apenas para PF
      paymentTerms,
      industry,
      observations,
    });
    alert('Cadastro de parceiro (lead) simulado! Verifique o console.');
    // Lógica para enviar para o backend virá aqui
    // Ex: await fetch('/api/register-partner', { method: 'POST', body: JSON.stringify(data) });
  };
  ...
  return (
    <div className="container">
      ...
      <form onSubmit={handleRegisterPartner}>
        ...
        <div className="form-group">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setContactName(''); // Limpar nome do contato PF
        ...
        {/* BLOCO PARA PESSOA JURÍDICA - COM ABAS INTERNAS (NÃO MEXER!) */}
        {partnerCategory === 'PJ' && (
          <div className="pj-tabs-container"> {/* Novo container para as abas PJ */}
            <h3>Dados da Pessoa Jurídica</h3>

            {/* Navegação das Abas da Pessoa Jurídica */}
            <div className="tabs pj-tabs"> {/* Adicionei 'pj-tabs' para estilização específica se precisar */}
              <button
                type="button"
            ...
            {activePjTab === 'pj_address' && (
              <div className="tab-content pj-tab-content">
                <h3>Endereço</h3> {/* Título para esta seção */}
                <div className="form-group">
        ...
        {partnerCategory === 'PF' && (
          <div className="pf-tabs-container">
            ...
            {activePfTab === 'pf_financial' && (
              <div className="tab-content pf-tab-content">
                <h3>Informações Financeiras</h3> {/* Título para esta seção */}
                <div className="form-group">
        ...
        {/* Removidos os campos comuns e o botão de submit final que estavam duplicados */}
        {/* Agora o botão de submit é renderizado dentro das últimas abas de PJ e PF */}
